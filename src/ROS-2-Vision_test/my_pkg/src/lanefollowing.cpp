...
void LaneFollowing::subs_callback(const Image::SharedPtr msg) {
  ...
  threshold(gray, gray, 153, 255, THRESH_BINARY);

  /*dst = gray(Rect(0, gray.rows / 3 * 2, gray.cols, gray.rows / 3));*/
  int start_x = gray.cols * 0;  // Start at one-third of the image width
  int start_y = gray.rows / 3 * 1; // Start at two-thirds of the image height (bottom third)
  int width = gray.cols;    // Select one-third of the image width
  int height = gray.rows / 3;   // Select one-third of the image height (bottom third)
  
  dst = gray(Rect(start_x, start_y, width, height));
...
void LaneFollowing::update_callback(){
  ...
  /**/
  /**/
  drive_msg.drive.steering_angle = ((error*90.0/150)/15) * M_PI / 180;
